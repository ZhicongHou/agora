...
class UserDiscussionSignal{
	 ...
	 */
	constructor(aappId,userName,role){
		this.role=role;
		this.roomId=null;
		this.appId=aappId;
		this.signal=null;
	...
	joinChatChannel(roomId,chatId){
		...
		channel.onChannelJoined = function(){
			...
			//
			// usignal.displayPersonList(roomId,amountId,personListId);
		}
		//用户加入失败
	 ...
	 * @return {[type]}          [description]
	 */
	displayChatMessage(labelId,account,message){
		if(message==="jushoushou2233"){
            $("#hand"+account).attr("src","/img/hand-green.png");
		}else if(message==="quxiaojushoushou2233"){
            $("#hand"+account).attr("src","/img/hand.png");
		}else if(message==="applyscreen3322"+this.userName){
            createStream();
        }else if(message.indexOf("applyscreen3322")>-1){

		}else if(message==="refreshmyfilefilefile23333"){
            refreshFiles();
        }else if(message==="refreshannouncementannouncementannouncement23333"){
            refreshAnnouncement();
        }else{
			$("#"+labelId).append("<li>" + account + " : " + message + "</li>");
		}
		...
		this.userSession.invoke("io.agora.signal.channel_query_userlist", {"name":userSignal.roomId},function(err,val){
            $("#amount").html(val.num-1);
    			...
    			for(var i = 0; i<list.length; i++){
    				if(list[i][0]!=userSignal.userName){
